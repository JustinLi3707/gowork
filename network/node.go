...
func NewNode(nodeID string) *Node {
	...
	node := &Node{
		...
		NodeTable: map[string]string{
			"Apple":  "localhost:8234",
			"MS":     "localhost:7001",
			"Google": "localhost:1000",
			"IBM":    "localhost:8237",
		},
		View: &View{
			ID: viewID,
...
func (node *Node) dispatchMsg() {
	for {
		...
		case msg := <-node.MsgEntrance:
			fmt.Println("channel waited msgentrancev msg!")
...
func (node *Node) routeMsg(msg interface{}) []error {
	...
	case *consensus.RequestMsg:
		if node.CurrentState == nil {
			...
			// Send messages.
			fmt.Println("----------RequestMsg:node.MsgDelivery <- msgs---------")
	...
	case *consensus.PrePrepareMsg:
		if node.CurrentState == nil {
			...
			// Send messages.
			fmt.Println("----------PrePrepareMsg:node.MsgDelivery <- msgs---------")
			node.MsgDelivery <- msgs
		} else {
	...
	case *consensus.VoteMsg:
		if msg.(*consensus.VoteMsg).MsgType == consensus.PrepareMsg {
			if node.CurrentState == nil || node.CurrentState.CurrentStage != consensus.PrePrepared {
				...
				// Send messages.
				fmt.Println("----------VoteMsg:node.MsgDelivery <- msgs---------")
				node.MsgDelivery <- msgs
			}
			...
			if node.CurrentState == nil || node.CurrentState.CurrentStage != consensus.Prepared {
				...
				// Send messages.
				fmt.Println("----------CommitMsg:node.MsgDelivery <- msgs---------")
				node.MsgDelivery <- msgs
			}
		}
	}
...
func (node *Node) resolveMsg() {
	for {
		...
		fmt.Println("get node.MsgDelivery!")
		switch msgs.(type) {
		case []*consensus.RequestMsg:
